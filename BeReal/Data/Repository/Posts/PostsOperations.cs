...
namespace BeReal.Data.Repository.Posts
{
    public class PostsOperations : IPostsOperations
    {
        ...
        public async Task<List<BR_Post>> GetUserPosts(BR_ApplicationUser user) => await _context.BR_Posts.Where(x => x.ApplicationUser!.Id == user.Id).ToListAsync();
        public async Task<List<BR_Post>> GetAllPosts() => await _context.BR_Posts.Include(x => x.Document).Include(x => x.ApplicationUser).Include(x => x.Comments).ToListAsync();
        public async Task<List<BR_Post>> GetPostsOfUser(string id) => await _context.BR_Posts.Include(x => x.Document).Include(x => x.ApplicationUser).Include(x => x.Comments).Where(x => x.ApplicationUser!.Id == id).ToListAsync();
        ...
        public async Task<BR_Post?> GetPostById(int id) => await _context.BR_Posts.FirstOrDefaultAsync(x => x.IDBR_Post == id);
        public async Task<BR_Post?> GetPostWithDocById(int id) => await _context.BR_Posts.Include(x => x.Document).FirstOrDefaultAsync(x => x.IDBR_Post == id);
